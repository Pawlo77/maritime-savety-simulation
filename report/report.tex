...
The scheduler runs at a fixed \emph{macro tick} of 15\,min ($\Delta t = 0.25$\,hr) for strategic and tactical logic. COLREGs manoeuvre decisions are therefore evaluated at macro-tick cadence. All stochastic draws share a single seeded random-number generator so that two runs with the same seed produce byte-identical KPI logs.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\columnwidth}{@{}lXr@{}}
        ...
        \midrule
        1 — Calm Passage &
        20 vessels, calm-biased weather profile, $\sigma_\text{shore}{=}0.05$.
        Pass: ${<}0.5$ collisions per 1\,k ship-hrs. &
        Calib. \\[4pt]
        2 — Storm Corridor &
        25 vessels (30\,\% Green crew); storm-focused weather profile;
        ...
        H3 \\[4pt]
        4 — Deep Water Rescue &
        15 vessels with endpoint-centred offshore spawn annulus ${>}60$\,nm;
        $W$ spikes to 0.90 at tick~30. &
        H2, H3 \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table*}[htbp]
    ...
    \begin{tabularx}{16cm}{@{}lX@{}}
        ...
        \midrule
        \texttt{n\_vessels} & Fleet size per run. Stress profile now supports up to 100 vessels. \\
        ...
        \texttt{shore\_broadcast\_radius\_nm}, \texttt{radio\_packet\_loss\_rate} & Shore coverage and stochastic communication degradation. \\
...
The target reference rate for calibration — fewer than 0.5 collisions per $1\,000$ ship-hours under Baseline~A in Scenario~1 (Table~\ref{tab:scenarios}) — is grounded in AIS-derived near-miss encounter statistics~\cite{silveira2013}. If H1 or H2 fail statistically, the mitigation is to recalibrate $\alpha$, $\beta$, $\gamma$ in Eq.~\ref{eq:evac} or widen $\sigma_\text{shore}$ before final reporting; design parameters were chosen conservatively to make this adjustment feasible without invalidating the comparison.
...
The simulation now supports higher-density long-horizon studies — up to 100 concurrent vessels and month-long (2880 tick) runs on the stress profile — while retaining a $50 \times 50$ weather grid for map legibility and interactive dashboard performance on commodity laptops. The framework remains extensible: lane geometry, agent counts, run horizon, weather severity, and $\sigma_\text{shore}$ are all constructor parameters, and additional experiment configurations can be added as factory functions without modifying any agent logic.
