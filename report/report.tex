...
\end{equation}
%
with $\alpha = 4.0$, $\beta = 2.5$, $\gamma = 1.8$. A high blended hazard estimate and a high preparedness score promote early evacuation; large forecast error suppresses it, preventing false alarms in noisy information conditions. The parameters are calibrated to produce an evacuation activation rate (KPI~5) of around 20\,\% in the high-hazard Scenario~2. On trigger, the vessel broadcasts an SOS packet and begins raft deployment.
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item \textbf{Regime switch.} A Bernoulli draw may flip the global regime: calm$\to$storm at probability $p_{cs}$ (default 0.03), storm$\to$calm at probability $p_{sc}$ (default 0.08). The asymmetry keeps storm episodes short relative to calm periods, in line with North-Sea base rates.

    \item \textbf{Storm cell lifecycle.} Up to \texttt{max\_systems} circular cells may be active. Each tick a new cell may spawn at probability $p_\text{spawn}\times r_\text{regime}$ ($r=1.35$ in storm, $0.65$ in calm). Active cells drift at 0.6 grid-cells/tick along a jittered bearing; radius oscillates sinusoidally; intensity decays faster in calm (0.020/tick) than in storm (0.006/tick). Cells below intensity 0.12 are removed.
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[x=0.085cm, y=1.0cm, >=Stealth, font=\scriptsize]
  \fill[blue!10] (0,-0.44) rectangle (50,0.44);
  \draw[blue!35, dashed] (50,-0.55) -- (50,0.55);
  \node[blue!60, font=\tiny, align=center] at (25,0.28) {shore coverage};
  \fill[gray!7] (50,-0.44) rectangle (90,0.44);
  \node[gray!65, font=\tiny] at (68,0.28) {radio shadow};
  \filldraw[black!80] (0,0) circle (2.2pt);
  \node[above, font=\tiny, black!80] at (0,0.12) {\textbf{S}};
  \foreach \x/\lbl/\col in {50/A/orange!90!black, 65/B/orange!75!black, 80/C/red!55!black} {
    \filldraw[\col] (\x,0) circle (2.4pt);
    \node[above, font=\tiny, \col] at (\x,0.12) {\lbl};
    \draw[\col!55, thin] (\x-15,-0.30) -- (\x+15,-0.30);
    \draw[\col!55, thin] (\x-15,-0.36) -- (\x-15,-0.24);
    \draw[\col!55, thin] (\x+15,-0.36) -- (\x+15,-0.24);
    \node[below, font=\tiny, \col!75] at (\x,-0.45) {15\,nm};
  }
  \draw[->, thick, blue!60] (2,0.12) -- (48,0.12)
      node[midway, above, font=\tiny]{direct ($\leq$50\,nm)};
  \draw[->, thick, orange!85!black] (52,0.12) -- (63,0.12)
      node[midway, above, font=\tiny]{hop 1};
  \draw[->, thick, orange!70!black] (67,0.12) -- (78,0.12)
      node[midway, above, font=\tiny]{hop 2};
  \draw[->] (-1,-0.72) -- (91,-0.72) node[right, font=\tiny]{nm};
  \foreach \x in {0,25,50,65,80}
    \draw[thin] (\x,-0.66) -- (\x,-0.78) node[below, font=\tiny]{\x};
\end{tikzpicture}
\caption{Two-hop relay coverage. Shore station~S covers out to 50\,nm. Vessel~A (at the shore edge) relays to~B (hop~1), B relays to~C (hop~2), extending effective weather data reach to 80\,nm — a 30\,nm gain over shore-only. Range bars show each vessel's 15\,nm radio neighbourhood; a third hop is suppressed by \texttt{max\_hop\_count}~=~2.}
\label{fig:relay}
\end{figure}
