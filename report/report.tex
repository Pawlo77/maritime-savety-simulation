...
The simulation occupies a $100 \times 100$\,nm continuous space implemented with the Mesa agent-based modelling framework~\cite{mesa2020}. The world includes an explicit static land mask configured as an island-archipelago layout: an irregular western mainland plus distributed offshore islands and shallow banks, used consistently in both runtime dynamics and dashboard rendering. Default traffic uses five intersecting non-linear waypoint routes designed for GUI usability and map interpretability: three west-east corridors (\texttt{west\_east\_southern}, \texttt{west\_east\_mid\_channel}, \texttt{west\_east\_northern\_arc}) and two north-south corridors (\texttt{north\_south\_west\_channel}, \texttt{north\_south\_central\_channel}). These defaults satisfy lane-validity constraints at startup (shore proximity, shoreline clearance, and end-point boundary/shore conditions), so initial dashboard runs do not require manual path repair. Shipping lanes are represented as ordered waypoint polylines constrained to open water (lane segments intersecting land are rejected at configuration time). The model supports one or more coastal stations; each station must be placed on land and broadcasts with radius $R_\text{shore} = 50$\,nm. Vessel initialisation is route-constrained: for each agent, one route endpoint is sampled (with configurable endpoint weights), then a spawn point is drawn from an annulus centred on that endpoint with radius bounds $[r_\text{min}, r_\text{max}]$ and an outer-ring sampling bias toward $r_\text{max}$.
...
The Streamlit dashboard provides an analysis workflow designed around experimental decision-making rather than static metric inspection. The lane-builder form was refactored into a compact editor with explicit lane status badges, consolidated validation feedback, and resettable valid default routes to reduce configuration friction in interactive use.
