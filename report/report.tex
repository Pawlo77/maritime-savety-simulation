...
\begin{description}
    \item[Experimental conditions.] All experiments compare three methods:
        \begin{description}[leftmargin=2.2em, labelindent=0pt]
            ...
            \item[Baseline~B (Shore-Only)] adds reception of the coastal station broadcast when in range, but has no peer relay and no confidence-weighted fusion.
...
The simulation occupies a $100 \times 100$\,nm continuous space implemented with the Mesa agent-based modelling framework~\cite{mesa2020}. Three pre-defined shipping lanes cross the world: a north--south freighter route, an east--west tanker route, and a diagonal coastal ferry route. A single coastal station is fixed at $(0,50)$\,nm — the western boundary — with a broadcast radius of $R_\text{shore} = 50$\,nm.
...
\begin{description}
    \item[Vessel agents] traverse the shipping lanes following pre-defined waypoints. Each maintains position, heading, speed, and crew archetype (Veteran / Standard / Green), together with accumulated fatigue (\texttt{hours\_awake}), a local weather observation at the vessel's current cell, a radio inbox of received mesh packets, and a preparedness score $P_\text{prep}$ (Section~\ref{sec:behaviour}) that gates all tactical decisions. State transitions follow: \texttt{ACTIVE} $\to$ \texttt{EVAC} $\to$ \texttt{SUNK} or \texttt{RESCUED}.

    \item[Coastal station agents] are stationary. Each tick they sample the true weather at their position, add Gaussian forecast noise (Eq.~\ref{eq:shoreNoise}), and broadcast to all vessels satisfying the receive probability criterion (Eq.~\ref{eq:Prx}). They also maintain an SOS dispatch queue and direct the nearest idle rescue asset on receipt of a distress call.
...
Every tick, the coastal station transmits a noisy estimate of the hazard at its location:
%
\begin{equation}
...
SOS packets follow the same relay path with priority pre-emption: on receipt, a vessel immediately re-broadcasts before processing its own weather logic, so SOS propagation latency is bounded to a single 15\,min macro tick regardless of the number of intermediate relays.
...
%
\begin{equation}
  \text{TTA} = \frac{d_\text{ship,shore}}{v_\text{rescue}}
               + \Delta t_\text{mob},
  \label{eq:tta}
\end{equation}
%
...
Collisions are detected using pairwise circle-overlap checks at a collision
radius of $r = 0.15$\,nm, consistent with near-miss studies on vessel
encounter geometry~\cite{zhang2015}.
Each intersection increments both vessels' damage counters; damage above
